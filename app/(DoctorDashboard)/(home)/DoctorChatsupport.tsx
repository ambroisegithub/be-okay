...
const DoctorChatSupport = () => {
  ...
  const [fistmodalVisible, setFirstModalVisible] = useState(false);

  const [male, female] = useState(null);
  const [gender, setgender] = useState("yes");
  const [selectedmedicationHistory, setSelectedmedicationHistory] = useState<string[]>([]);
...
const router = useRouter();
  ...
  const medicationHistory: string[] = [
    "Allergies",
    "Previous Surgeries",
    "Family illness",
    "Chronic illness",
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal
        visible={fistmodalVisible}
        animationType="slide"
        transparent={true}
        onRequestClose={() => setFirstModalVisible(false)}
      >
        <View style={styles.modalBackground2}>{renderModalContent()}</View>
      </Modal>
