...
const ConsultationWithAI = () => {
  ...
  const [painDurationUnit, setPainDurationUnit] = useState<DurationUnit>("days");
  const router = useRouter()

  type DurationUnit = 'days' | 'months' | 'years' | 'decades';

  const unitLimits: Record<DurationUnit, { min: number; max: number }> = {
    days: { min: 1, max: 30 },
    months: { min: 1, max: 12 },
    years: { min: 1, max: 10 },
    decades: { min: 1, max: 5 },
  ...
  const navigateToBack = () => {
    router.push("/(PatientDashboard)/(home)/Consultation");
  };
  ...
  const navigateToConsultationResult=()=>{
    router.push("/(PatientDashboard)/(home)/ConsultationResult")
  }
  return (
              ...
              <View style={styles.picker}>
                <Picker
                  ...
                  onValueChange={(itemValue) => {
                    const value = itemValue as DurationUnit; 
                    setPainDurationUnit(value);
                    setPainDuration(unitLimits[value].min);
                  }}
                >
